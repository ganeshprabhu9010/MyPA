...
function getUpComingEventsCount() {
    ...
    var startDate = new Date(new Date().getTime() + 0*24*60*60*1000);
...
function getListOfEvents(startDate,endDate,success,error) {
    window.plugins.calendar.listEventsInRange(startDate,endDate,success,error);
}

function setStartDayFor(date) {
    return new Date(date.setHours(0,0,0,0));
}

function setEndDayFor(date) {
   return new Date(date.setHours(23,59,59,999));
}

function createCalendarEvent(eventDetails,success,error) {
    ...
    var calOptions = null;// eventDetails["calOptions"];
    if(calOptions == null) {
